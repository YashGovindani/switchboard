...
- **Templates** — create one directly with **New Template** in the header (same chat-driven builder, edit ✏️ / delete 🗑 from the template list), or snapshot an existing environment with the ⧉ button in the builder header (saving again overwrites). The new-environment screen offers **"start from a template"**: select one, choose a name, press Enter — the environment is pre-filled, cached commands included, so nothing is re-translated.
