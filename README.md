...
- **Templates** — create one directly with **New Template** in the header (same chat-driven builder, edit ✏️ / delete 🗑 from the template list), or snapshot an existing environment with the ⧉ button in the builder header (saving again overwrites). Both creation screens offer **"start from templates"**: select any number and their actions combine into the new environment or template. Everything is deep-copied — fresh action records, duplicated chat histories, copied cached commands — so editing the copy never touches the source templates.
